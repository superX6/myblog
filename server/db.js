...
loginSchema.statics.createNewAccount= function(doc, callback) {
  return this.model('mongoose').create(doc, callback);
}
loginSchema.statics.findUser= function({account: account,password: password}, callback) {
  return this.model('mongoose').find({account: account,password: password}, callback);
}

/************** 定义模型Model **************/

var LoginModel = mongoose.model('mongoose',loginSchema)
